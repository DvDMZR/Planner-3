...
function App() {
  ...
  const setIsAssignModalOpen = useCallback(val => {
    if (val && !currentUserRef.current) {
      showToastRef.current?.('Bitte anmelden, um Änderungen vorzunehmen');
      return;
    }
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (parsedData) {
        ...
        if (parsedData.appUsers && parsedData.appUsers.length > 0) setAppUsers(parsedData.appUsers);else setAppUsers([{
          id: 'default-admin',
          name: 'Admin',
          pin: '1234',
          role: 'admin'
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-slate-800 text-white text-sm px-4 py-2.5 rounded-lg shadow-lg flex items-center gap-2 pointer-events-none select-none"
